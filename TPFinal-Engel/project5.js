...
//default clear color
var ClC = {r:.3,g:.3,b:.4,a:1};

let t = 0;
function generate_tree(){
	meshDrawer.generate_tree(t);
	t++;
}
...
function UpdateProjectionMatrix()
{
	...
	meshDrawer.proj = perspectiveMatrix;
}
...
function DrawScene()
{
	...
	gl.clearColor( ClC.r, ClC.g, ClC.b, ClC.a);
	gl.clear( gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT );
	
	// 3. Le pedimos a cada objeto que se dibuje a si mismo
	var nrmTrans = [ mv[0],mv[1],mv[2], mv[4],mv[5],mv[6], mv[8],mv[9],mv[10] ];
	meshDrawer.transX = transX;
	meshDrawer.transY = transY;
	meshDrawer.transZ = transZ;
	meshDrawer.rotX = rotX;
	meshDrawer.rotY = rotY+autorot;
	...
	canvas.onmousedown = function() 
	{
		...
		if ( event.ctrlKey ) 
		{
			canvas.onmousemove = function() 
			{
				...
				canvas.pan(event.clientX - cx, event.clientY - cy);
				cy = event.clientY;
				cx = event.clientX;
			}
		}
